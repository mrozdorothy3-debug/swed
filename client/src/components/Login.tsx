...
const Login: React.FC = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    // Redirect based on role handled in AppRoutes
      ...
      <Card sx={{ width: '100%', maxWidth: 420, boxShadow: '0 8px 32px rgba(0,0,0,0.08)', borderRadius: 3 }}>
        <CardContent sx={{ p: 4 }}>
          ...
          <Box component="form" onSubmit={onSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <TextField label="Full Name" value={username} onChange={(e) => setUsername(e.target.value)} fullWidth required placeholder="e.g. John Smith" helperText="Enter your first and last name" />
